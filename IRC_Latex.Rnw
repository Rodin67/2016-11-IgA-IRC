...
<<results="asis">>=
x <- coxph(Surv(kmdelaidc/365.25, kmdc)~age+sex+dfg, data = global_greffe[global_greffe$liste_longue=="Néphropathie à dépôts d'IgA",])
print(xtable(x), table.placement = "H")
@

Cox sur age + sexe + dfg + albini :
<<results="asis">>=
x <- coxph(Surv(kmdelaidc/365.25, kmdc)~age+sex+dfg+ALBINI, data = global_greffe[global_greffe$liste_longue=="Néphropathie à dépôts d'IgA",])
print(xtable(x), table.placement = "H")
@
