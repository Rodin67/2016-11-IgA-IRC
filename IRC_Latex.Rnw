...
<<options et library, echo=FALSE, message=FALSE>>=
...
greffe <- greffe[!duplicated(greffe$NEFG),] # On supprime les patients avec le même identifiant (sans données)
...
La table "globale" \Sexpr{nrow(global)} patients différents ayant une IRCT avec maladie de Berger (\Sexpr{nrow(iga)}), une néphropathie diabétique (\Sexpr{nrow(diab)}) ou une glomérulonéphrite chronique (\Sexpr{nrow(gnc)}).

La table "greffe" contient \Sexpr{nrow(greffe)} patients, dont \Sexpr{length(intersect(global$RREC_COD,greffe$NEFG))} (\Sexpr{round(length(intersect(global$RREC_COD,greffe$NEFG))/nrow(greffe)*100,1)}\%) qui sont en commun avec la table "globale". \Sexpr{length(intersect(global$RREC_COD[is.na(global$dgrf)],greffe$NEFG))} patients sont dans la table "greffe" alors qu'ils n'ont pas de date de greffe dans la table "globale".
...
chisq.test(table(iga$anirt),correct = F)
@
