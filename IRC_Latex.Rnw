...
xtable(standdirect[order(standdirect$Ratio.ajuste, decreasing = T),]*10)
@
...
<<cache=FALSE>>=
standdirect_dep[order(standdirect_dep$Ratio.ajuste, decreasing = T),]*10
@
...
<<fig.height=4>>=
par(mar=c(4,4,3,4))
hist(iga$age, main =  "Répartition des âges avec courbe de densité", xlab = "Âge", ylab = "Fréquence")
par(new=T)
plot(density(iga$age), xaxt = "n", yaxt = "n",xlab="",ylab="", main = "",type="l", col = "red")
axis(4, ylim = c(0,0.020), col="red",col.axis="red")
mtext("Densité",side=4,col="red",line=2.5)
@
...
<<fig.height=5>>=
tapply(iga$age, iga$gr_bmi, FUN = summary)
ggplot(iga, aes(x = gr_bmi, y = age)) + 
  geom_boxplot(fill = "grey80", colour = "black") +
  xlab("BMI") +
  ylab("Âge lors de l'IRT pour la maladie de Berger")

plotmeans(iga$age~iga$gr_bmi, gap=0, mean.labels= T)
...
plotmeans(global$age~global$gr_bmi, gap=0)
...
<<results="asis">>=
print(xtable(addmargins(table(global$liste_longue, global$PBRn, useNA = "always")[-14,])), table.placement = "H")

x <- round(prop.table(table(global$liste_longue, global$PBRn),1)*100,1)
print(xtable(x[order(x[,2], decreasing = T),]), table.placement = "H")
@
...
<<results="asis">>=
xtable(addmargins(table(iga$RES_REG_LIB, iga$PBRn, useNA = "ifany")))
x <- round(prop.table(table(iga$RES_REG_LIB, iga$PBRn),1)*100,1)
print(xtable(x[order(x[,2], decreasing = T),], caption = "Pourcentage"), table.placement = "H")
@
...
<<>>=
print("Cirrhose et VHB :")
table(iga$VHBn[iga$CIRHn==1])
print("Cirrhose et VHC :")
table(iga$VHCn[iga$CIRHn==1])
@
...
table(global$avc_ait, useNA = "always")
@
...
<<results="asis">>=
print(xtable(do.call("rbind", tapply(global$dfg, global$liste_longue, summary)), digits = 1), size = "small")
@
...
summary(glm(iga$gr_HBINI~iga$sex+iga$dfg, family = "binomial"))
paste("OR d'être de sexe féminin :", round(exp(-0.8766 ),2))
@
...
xtable(inc_greffe, digits = 1)
apply(table(iga$METHOn, iga$anirt)[1:2,], 2 ,sum)
apply(table(gnc$METHOn, gnc$anirt)[1:2,], 2 ,sum)
apply(table(diab$METHOn, diab$anirt)[1:2,], 2 ,sum)
apply(table(pkrd$METHOn, pkrd$anirt)[1:2,], 2 ,sum)
@
...
addmargins(table(iga$URGn,iga$REAn, useNA = "always", deparse.level = 2))
table(iga$urg_rea, useNA = "always")
@
...
<<results="asis">>=
print(xtable(sort(table(iga$ACTIVn, useNA = "always")[-11], decreasing = T)), include.colnames = F)
xtable(round(prop.table(sort(table(iga$ACTIVn), decreasing = T))*100,1), digits = 1)
@
  ...
  On a \Sexpr{table(iga$dc)[2]} (\Sexpr{round(table(iga$dc)[2]/nrow(iga)*100,1)}\%) patients qui sont décédés. On a la date de décès pour \Sexpr{table(!is.na(iga$ddc))[2]} (\Sexpr{round(table(!is.na(iga$ddc))[2]/nrow(iga)*100,1)}\%) patients.

<<>>=
print("En ligne : décédé ; en colonne : présence d'une date de décès")
...
xtable(inc_greffe_iga, digits = 1) 
...
<<>>=
sort(table(global_greffe$ECHEC1, useNA = "always"), decreasing = T)
print("Pourcentage :")
round(prop.table(sort(table(global_greffe$ECHEC1), decreasing = T))*100,1)
@
