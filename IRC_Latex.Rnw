...
print(xtable(x, auto = T, caption = "Nombre de PBR par région"), table.placement = "H", include.rownames = F)
@
...
summary(lm(age ~ gr_bmi + PDS + DIABn + IDMn + AMIn + sex, data = iga))

ggplot(iga[iga$bmi<46 & iga$bmi>13,], aes(x=bmi, y=age)) +
  geom_smooth() +
  labs(title = "Régression - maladie de Berger")
summary(lm(age ~ bmi + PDS + DIABn + IDMn + AMIn + sex, data = iga))
...
ggplot(diab[diab$bmi<46 & diab$bmi>13,], aes(x=bmi, y=age)) +
  geom_smooth() +
  labs(title = "Régression - diabète (bmi<46)")
ggplot(pkrd[pkrd$bmi<46 & pkrd$bmi>13,], aes(x=bmi, y=age)) +
  geom_smooth() +
  labs(title = "Régression- pkrd (bmi<46)")
