...
iga$NOINSCn <- factor(iga$NOINSCn, labels = c("CI médicale","Refus patient","autre/non décrit","Bilan pré_greffe en cours"))
...
greffe$kmdelaidc<- ifelse(is.na(greffe$DECES1), as.Date(c("2016-11-01"), origin = "1899-12-30")-greffe$GRF1, greffe$DELAIDC1) # Pour faire un Kaplan-Meier : délai avant évènement (ou suivi jusqu'au 1-11-2016)
...
print(xtable(standdirect[order(standdirect$Ratio.ajuste, decreasing = T),]*10, digits = 1), table.placement = "H")
@
...
<<results="asis">>=
x <- standdirect_dep[order(standdirect_dep$Ratio.ajuste, decreasing = T),]*10
print(xtable(x[c(1:50),], digits = 1), table.placement = "H")
print(xtable(x[c(51:99),], digits = 1), table.placement = "H")
@
...
<<results="asis">>=
inc_iga <- matrix(nrow = 4, ncol = 5, dimnames = list(c("Berger","Non Berger","Total France","Cas pour 1M d'hab"),c("2010","2011","2012","2013","2014")))
inc_iga[1,] <- c(322, 347, 358, 360, 333)
inc_iga[2,] <- c(50482295-322, 50701229-347, 50930784-358, 51192770-360, 51421008-333)
inc_iga[3,] <- c(50482295, 50701229, 50930784, 51192770, 51421008)
inc_iga[4,] <- round(prop.table(inc_iga[1:2,],2)*1000000,1)[1,]
print(xtable(inc_iga, auto = T), table.placement = "H")
@

<<>>=
chisq.test(inc_iga[1:2,], correct = F)
@
...
Sexe (2010-2014) : Hommes \Sexpr{table(iga$sex, useNA = "always")[1]} (\Sexpr{round(prop.table(table(iga$sex))*100,1)[1]}\%), femmes \Sexpr{table(iga$sex, useNA = "always")[2]} (\Sexpr{round(prop.table(table(iga$sex))*100,1)[2]}\%)
<<sexe, results="asis">>=
...
summary(lm(iga$age~iga$gr_bmi))

ggplot(iga, aes(x=bmi, y=age)) +
  geom_smooth() +
  labs(title = "Régression - maladie de Berger")
summary(lm(iga$age~iga$bmi))
...
print(xtable(tidy(summary(lm(global$age~global$gr_bmi))$coefficients), digits = 3), table.placement = "H")
@
...
ggplot(diab[diab$bmi<60,], aes(x=bmi, y=age)) +
  geom_smooth() +
  labs(title = "Régression - diabète (bmi<60)")
ggplot(pkrd[pkrd$bmi<60,], aes(x=bmi, y=age)) +
  geom_smooth() +
  labs(title = "Régression- pkrd (bmi<60)")

gnc$liste_longue <- factor(gnc$liste_longue)
for (i in levels(gnc$liste_longue)) {
print(ggplot(gnc[gnc$liste_longue==i,], aes(x=bmi, y=age)) +
  geom_smooth() +
...
<<results="asis">>=
print(xtable(addmargins(table(global$liste_longue, global$PBRn, useNA = "always")[-14,]), digits = 0), table.placement = "H")
...
<<results="asis">>=
print(xtable(addmargins(table(iga$RES_REG_LIB, iga$PBRn, useNA = "ifany")), digits = 0), table.placement = "H")
x <- round(prop.table(table(iga$RES_REG_LIB, iga$PBRn),1)*100,1)
print(xtable(x[order(x[,2], decreasing = T),], caption = "Pourcentage", digits = 1), table.placement = "H")
@
...
<<>>=
addmargins(table(global$liste_longue, global$CIRHn, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$CIRHn),1)*100,1)
x[order(x[,2], decreasing = T),]
@

Stade de la cirrhose :

- 1 : \Sexpr{table(iga$STDCIRHn,useNA = "always")[2]} (\Sexpr{round(prop.table(table(iga$STDCIRHn[!iga$STDCIRHn==0]))*100,1)[1]}\%)

- 2 : \Sexpr{table(iga$STDCIRHn,useNA = "always")[3]} (\Sexpr{round(prop.table(table(iga$STDCIRHn[!iga$STDCIRHn==0]))*100,1)[2]}\%)

<<>>=
addmargins(table(global$liste_longue[!global$STDCIRHn==0], global$STDCIRHn[!global$STDCIRHn==0], useNA = "always"))
...
<<>>=
addmargins(table(global$liste_longue, global$IDMn, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$IDMn),1)*100,1)
x[order(x[,2], decreasing = T),]
@
...
<<>>=
addmargins(table(global$liste_longue, global$ICn, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$ICn),1)*100,1)
x[order(x[,2], decreasing = T),]
@

Stade de l’IC :

- 1 : \Sexpr{table(iga$STADICn,useNA = "always")[2]} (\Sexpr{round(prop.table(table(iga$STADICn[!iga$STADICn==0]))*100,1)[1]}\%)

- 2 : \Sexpr{table(iga$STADICn,useNA = "always")[3]} (\Sexpr{round(prop.table(table(iga$STADICn[!iga$STADICn==0]))*100,1)[2]}\%)
~\\
...
<<>>=
addmargins(table(global$liste_longue, global$AMIn, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$AMIn),1)*100,1)
x[order(x[,2], decreasing = T),]
@

Stade de l’AOMI : 

- 1 : \Sexpr{table(iga$STDAMIn,useNA = "always")[2]} (\Sexpr{round(prop.table(table(iga$STDAMIn[!iga$STDAMIn==0]))*100,1)[1]}\%)

- 2 : \Sexpr{table(iga$STDAMIn,useNA = "always")[3]} (\Sexpr{round(prop.table(table(iga$STDAMIn[!iga$STDAMIn==0]))*100,1)[2]}\%)
~\\

Amputation : \Sexpr{table(iga$AMPn)[2]} (\Sexpr{round(prop.table(table(iga$AMPn))*100,1)[2]}\%) NA = \Sexpr{table(iga$AMPn, useNA = "always")[3]}

<<>>=
addmargins(table(global$liste_longue, global$AMPn, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$AMPn),1)*100,1)
x[order(x[,2], decreasing = T),]
@
~\\
...
<<>>=
iga$avc_ait <- NA
iga$avc_ait <- ifelse(iga$AVCn==1 | iga$AITn==1, 1, 0)
@
...
addmargins(table(global$liste_longue, global$avc_ait, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$avc_ait),1)*100,1)
x[order(x[,2], decreasing = T),]
@

VIH : \Sexpr{table(iga$VIHn,useNA = "always")[2]} (\Sexpr{round(prop.table(table(iga$VIHn))*100,1)[2]}\%)
<<>>=
addmargins(table(global$liste_longue, global$VIHn, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$VIHn),1)*100,1)
x[order(x[,2], decreasing = T),]
@

Dont stade SIDA : \Sexpr{table(iga$SIDAn)[2]} (\Sexpr{round(table(iga$SIDAn)[2]/table(iga$VIHn)[2]*100,1)}\%)
<<>>=
addmargins(table(global$liste_longue, global$SIDAn, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$SIDAn),1)*100,1)
x[order(x[,2], decreasing = T),]
@
~\\
...
<<>>=
addmargins(table(global$liste_longue, global$VHBn, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$VHBn),1)*100,1)
x[order(x[,2], decreasing = T),]
@
~\\
...
<<>>=
addmargins(table(global$liste_longue, global$VHCn, useNA = "always"))
x <- round(prop.table(table(global$liste_longue, global$VHCn),1)*100,1)
x[order(x[,2], decreasing = T),]
@
...
<<results="asis">>=
print(xtable(addmargins(table(global$liste_longue, global$TABACn, useNA = "always"))[-14,], digits = 0), table.placement ="H")
...
@
On a \Sexpr{length(iga$dfg[iga$dfg=="123.02" & !is.na(iga$dfg)])} patient qui a un DFG supérieur à 120 ml/min. Aucun entre 40 et 120.

<<results="asis">>=
x <- do.call("rbind", tapply(global$dfg, global$liste_longue, summary))
x <- as.data.frame.matrix(x)
x <- x[order(x$Mean, decreasing = T),]
print(xtable(x, digits = 1), size = "small", table.placement = "H")
@
...
<<results="asis">>=
print(xtable(tidy(table(iga$ALBI_MTH,useNA = "always"))), include.rownames = F, table.placement = "H")
print(xtable(round(prop.table(table(iga$ALBI_MTH))*100,1)), table.placement = "H")
@
...
<<results="asis">>=
levels(iga$METHOn) <- c("Hémodialyse","Dialyse péritonéale", "Greffe sans dialyse")
xtable(addmargins(table(iga$METHOn, iga$anirt)), digits = 0)
xtable(round(prop.table(table(iga$METHOn, iga$anirt),2)*100,1), caption = "Pourcentage", digits = 1) # Greffe = greffé sans être passé par la dialyse
@

<<>>=
chisq.test(table(iga$METHOn, iga$anirt))
@

<<results="asis">>=
global$liste_longue <- factor(global$liste_longue)
for (i in levels(global$liste_longue)) {
  ...
  print(xtable(round(prop.table(table(global$METHOn[global$liste_longue==i], global$anirt[global$liste_longue==i]),2)*100,1), caption = paste("Pourcentage",i), digits = 1), table.placement="H")
} 
@


\textcolor{red}{Nombre de patients greffés direct n'ayant aucune info sur la greffe toute pathologies confondus) : \Sexpr{table(is.na(global_greffe$GRF1[global_greffe$METHOn=="Greffe"]))[2]} (\Sexpr{round(prop.table(table(is.na(global_greffe$GRF1[global_greffe$METHOn=="Greffe"])))*100,1)[2]}\%)}
~\\
...
inc_greffe <- matrix(nrow = 9, ncol = 5, dimnames = list(c("Total France","Berger","Pourcentage Berger","GNC","Pourcentage GNC","Diabète","Pourcentage diabète","PKRD","Pourcentage PKRD"),c("2010","2011","2012","2013","2014")))
...
<<results="asis">>=
x <- left_join(tidy(sort(table(iga$ACTIVn, useNA = "always")[-11], decreasing = T)), tidy(round(prop.table(sort(table(iga$ACTIVn), decreasing = T))*100,1)), by = "Var1")
colnames(x) <- c("Activité","Fréquence","Pourcentage")
print(xtable(x), include.rownames = F, table.placement = "H")
@
...
plot(survfit(Surv(kmdelaidc/365.25, kmdc)~liste_longue, data = global_greffe),
     ...
     xlab = "Temps (année)", col = c("black",brewer.pal(n = 12, name = "Paired")))
legend(x = 0, y = 0.5, legend = c(levels(global_greffe$liste_longue)), lty = 1, col = c("black",brewer.pal(n = 12, name = "Paired")), cex = 1)
@
...
<<results="asis">>=
x <- global_greffe %>% group_by(tx, !is.na(dgrf),dgrf<as.Date("2015-01-01", origin ="1899-12-30") ,METHOn=="Greffe", !is.na(GRF1), !is.na(GRF2)) %>% summarise(n())
x <- as.data.frame(x)
colnames(x) <- c("Greffé", "Date de greffe", "Date 2010-2014","Greffe sans dialyse","Greffe 1", "Greffe 2","Total")
print(xtable(x, digits = 0, align = c("c","c","c","c","c","c","c","c")), table.placement = "H", include.rownames = F)
@
...
- ayant une date de greffe 1 et 2, considéré comme greffé et ayant une date de greffe de 2010 à 2014 : \Sexpr{table(global_greffe$greffe[global_greffe$dgrf<as.Date("2015-01-01", origin ="1899-12-30")])} (\Sexpr{round(table(global_greffe$greffe[global_greffe$dgrf<as.Date("2015-01-01", origin ="1899-12-30")])/20455*100,1)}\%)
...
print(xtable(inc_greffe_iga, digits = 1), table.placement = "H")
@
...
print(xtable(x, digits = 1, auto = T), table.placement = "H")
...
ggplot(global_greffe, aes(DELAIARF1/12)) + 
...
\Sexpr{length(global_greffe$DECES1[!is.na(global_greffe$DECES1)])} (\Sexpr{round(length(global_greffe$DECES1[!is.na(global_greffe$DECES1)])/length(intersect(global$RREC_COD, greffe$NEFG))*100,1)}\%) patients sont décédés au cours de la greffe 1 et \Sexpr{length(global_greffe$DECES2[!is.na(global_greffe$DECES2)])} au cours de la 2e.
