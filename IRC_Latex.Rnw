...
iga$CREATINI[iga$ICn==0 & iga$dfg>15] <- NA
...
<<greffe>>=
greffe.iga <- read_excel("~/IRC/dataIgA/greffe IgA.xlsx", 
                     col_types = c("numeric", "text", "date", "date", "date", "date",
                                   "date", "text", "text", "date", "date", "text",
                                   "text", "numeric", "text", "text", "text", "text", 
                                   "text", "text", "numeric"))
greffe.iga <- greffe.iga[greffe.iga$LMALADI=="Néphro. à dépôts d'IgA (diagnostic prouvé par immunofluorescence)" | greffe.iga$LMALADI=="Maladie de Berger",]
...
greffe <- bind_rows(greffe.iga, greffe.diab)
...
greffe.pkrd <- read_excel("~/IRC/dataIgA/grf_tem_pkr.xlsx", 
                          col_types = c("numeric", "text", "date", "date", "date", "date",
                                        ...
                                        "text", "numeric", "text", "text", "text",
                                        "text", "text", "text", "numeric"))
greffe.pkrd <- greffe.pkrd[greffe.pkrd$LMALADI=="Maladie polykystique rénale adulte (type dominant)",]

greffe <- bind_rows(greffe, greffe.pkrd)

greffe.gnc <- read_excel("~/IRC/dataIgA/grf_tem_gnc.xlsx", 
                          col_types = c("numeric", "text", "date", "date", "date", "date",
                                        "date", "blank", "text", "text", "blank",
                                        "date", "date", "text", "text", "numeric", "text",
                                         "text","blank", "text", "text", "text", "text",
                                        "numeric"))
greffe.gnc <- greffe.gnc[greffe.gnc$LMALADI=="Glomérulo. avec hyalinose segmentaire et focale de l'adulte (av.syndrome néphrotique)." | 
                           greffe.gnc$LMALADI=="Synd. néphrotique sévère avec sclérose focale (Pédiatrique, GN avec hyalinose segment. et focale de l'enfant)" | 
                           greffe.gnc$LMALADI=="Glomérulo. extra-membraneuse" | 
                           greffe.gnc$LMALADI=="Glomérulo. membrano-prolif., type I (par immunofluo. et/ou microscopie électr. (hors codes 784 ou 789)" | 
                           greffe.gnc$LMALADI=="Glomérulo. membrano-proliférative type II avec dépôts denses (prouvée par immunofluorescence et/ou microscopie électronique)" |
                           greffe.gnc$LMALADI=="Syndrome de Goodpasture" |
                           greffe.gnc$LMALADI=="Granulomatose de Wegener" |
                           greffe.gnc$LMALADI=="Polyangéite microscopique et glomerulonephrite extracapillaires pauci-immune à ANCA" |
                           greffe.gnc$LMALADI=="Lupus érythémateux" |
                           greffe.gnc$LMALADI=="Purpura rhumatoide",]
...
gnc$CREATINI[gnc$ICn==0 & gnc$dfg>15] <- NA
...
gnc <- gnc[!(gnc$liste_longue=="GN primitive avec autre diagnostic histologique" | gnc$liste_longue=="Maladies systémiques autres" | gnc$liste_longue=="GN primitive sans examen histologique"),]
...
diab$CREATINI[diab$ICn==0 & diab$dfg>15] <- NA
...
pkrd$CREATINI[pkrd$ICn==0 & pkrd$dfg>15] <- NA
...
global$CREATINI[global$dfg>20] <- NA
global$CREATINI[global$ICn==0 & global$dfg>15] <- NA
global$dfg[is.na(global$CREATINI)] <- NA
global <- global %>%
            mutate(iga = ifelse(liste_longue=="Néphropathie à dépôts d'IgA", 1, 0),
                   diab = ifelse(liste_longue=="Diabète", 1, 0),
                   pkrd = ifelse(liste_longue=="Polykystose rénale de l adulte", 1, 0),
                   gnc = ifelse(liste_longue=="GN avec HSF" | liste_longue=="GN extra-membraneuse" | liste_longue=="GN extracapillaire ou endo/extracapillaire" | liste_longue=="GN membrano-proliférative type 1" | liste_longue=="GN membrano-proliférative type 2, dépôts denses" | liste_longue=="GN primitive sans examen histologique" | liste_longue=="Néphropathie lupique" | liste_longue=="Purpura rhumatoïde", 1, 0))
@
...
global_greffe$ECHEC1<-factor(global_greffe$ECHEC1, 
                             labels = c("000",
                                          ...
                                          "Complications vasculaires du greffon",
                                          "Défaillance non spécifique du greffon",
                                          "Autres complications greffon",
                                          "Perte d'un greffon sur Bigreffe",
                                          "Ischémie myocardique ou infarctus",
                                          "Hémorragie d'origine chirurgicale",
                                          "Hémorragie au niveau du transplant",
                                          "Maladie kystique du foie",
                                          "Péritonite",
                                          "Insuffisance rénale aigue",
                                          "Insuffisance rénale chronique",
                                          "Patient refusant de poursuivre son traitement",
                                          "Infection bactérienne pulmonaire (sauf tuberculose)",
                                          "Infection virale d'autres organes (sauf hépatite virale)",
                                          "Infection fongique ou à protozoaire d'autres organes",
                                          "Septicémie",
                                          "Autre infection déterminee",
                                          "Cachexie",
                                          "Autre complication non déterminée",
                                          "Autre complication déterminée",
                                          "Cause inconnue"))
global_greffe$ECHEC2<-factor(global_greffe$ECHEC2, 
                             labels = c("Défaillance primaire",
                                        "Rejet aigu",
                                        "Complications vasculaires du greffon"))
...
global_greffe2$ECHEC1 <-factor(global_greffe2$ECHEC1, 
                               labels = c("000",
                                          ...
                                          "Récidive de la maladie initiale",
                                          "Complications vasculaires du greffon",
                                          "Complications urologiques greffon",
                                          "Défaillance non spécifique du greffon",
                                          "Autres complications greffon",
                                          "Perte d'un greffon sur Bigreffe",
                                          "Ischémie myocardique ou infarctus",
                                          ...
                                          "Autre complication déterminée",
                                          "Cause inconnue"))
global_greffe2$ECHEC2<-factor(global_greffe2$ECHEC2, 
                              labels = c("Défaillance primaire",
                                         "Rejet aigu",
                                         "Complications vasculaires du greffon"))
...
Au total, nous avons \Sexpr{table(duplicated(global_greffe$RREC_COD))["FALSE"]} patients différents étant passés au stade d'IRCT (\textgreater 16ans, entre 2010 et 2014) dont \Sexpr{table(duplicated(iga$RREC_COD))["FALSE"]} (\Sexpr{round(table(duplicated(iga$RREC_COD))["FALSE"]/table(duplicated(global$RREC_COD))["FALSE"]*100,1)}\%) avec maladie de Berger, \Sexpr{table(duplicated(diab$RREC_COD))["FALSE"]} (\Sexpr{round(table(duplicated(diab$RREC_COD))["FALSE"]/table(duplicated(global$RREC_COD))["FALSE"]*100,1)}\%) avec une néphropathie diabétique, \Sexpr{table(duplicated(gnc$RREC_COD))["FALSE"]} (\Sexpr{round(table(duplicated(gnc$RREC_COD))["FALSE"]/table(duplicated(global$RREC_COD))["FALSE"]*100,1)}\%) avec une glomérulonéphrite chronique et \Sexpr{table(duplicated(pkrd$RREC_COD))["FALSE"]} (\Sexpr{round(table(duplicated(pkrd$RREC_COD))["FALSE"]/table(duplicated(global$RREC_COD))["FALSE"]*100,1)}\%) avec une PKRD.
...
<<results="asis">>=
standdirect[order(standdirect$annuel, decreasing = T),] %>%
  `*`(10) %>%
  xtable(digits = 1) %>%
  print(table.placement = "H", size = "\\small")
@
...
title("Incidence d'IRCT Berger/1M habitants/an")
...
<<sexe, results="asis">>=
table(iga$sex, iga$anirt) %>%
  xtable(col.names = c("H","F")) %>%
...
<<fig.height=3>>=
ggplot(iga, aes(x = age, fill = anirt, col = anirt)) + 
...
<<taille, results="asis">>=
print(xtable(tidy(summary(iga$TAIL)), auto = T), include.rownames = F)
@

Poids lors de l'IRCT :

<<poids, results="asis">>=
print(xtable(tidy(summary(iga$PDS)), auto = T), include.rownames = F)
@

<<fig.height=3.5>>=
ggplot(iga, aes(x = PDS, y = dfg)) +
  geom_point(aes(col = as.factor(PBRn)), alpha = 0.6) +
...
<<fig.height=3.5>>=
ggplot(diab, aes(x = PDS, y = dfg)) +
  geom_point(aes(col = as.factor(PBRn)), alpha = 0.5) +
  geom_smooth(method = "lm",se = FALSE) +
  theme(legend.position = "bottom") +
  ggtitle("Diabète")
@
...
<<fig.height=12>>=
ggplot(gnc, aes(x = PDS, y = dfg)) +
  geom_point(aes(col = as.factor(PBRn)), alpha = 0.5) +
  geom_smooth(aes(col = as.factor(PBRn)),method = "lm",se = FALSE) +
  theme(legend.position = "bottom") +
  facet_grid(liste_longue~.)
@
...
x[2,6] <- NA
rownames(x) <-  c("Effectif","Pourcentage")
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$PBRn, useNA = "ifany")[-14,]) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$PBRn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","Sum","Non%","Oui%")
print(xtable(x[order(x[,5]),], digits = 1), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$DIABn, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$DIABn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","NA","Sum","Non%","Oui%")
print(xtable(x[order(x[,6]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
% 
% Type de diabète : 
% 
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$CIRHn, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$CIRHn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","NA","Sum","Non%","Oui%")
print(xtable(x[order(x[,6]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue[!global$STDCIRHn==0], global$STDCIRHn[!global$STDCIRHn==0], useNA = "no")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue[!global$STDCIRHn==0], global$STDCIRHn[!global$STDCIRHn==0]),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Child A","Child B-C","Sum","Non%","Oui%")
print(xtable(x[order(x[,5]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@

\subsubsection*{Infarctus du myocarde}

<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$IDMn, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$IDMn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","NA","Sum","Non%","Oui%")
print(xtable(x[order(x[,6]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$ICn, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$ICn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","NA","Sum","Non%","Oui%")
print(xtable(x[order(x[,6]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue[!global$STADICn==0], global$STADICn[!global$STADICn==0], useNA = "no")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue[!global$STADICn==0], global$STADICn[!global$STADICn==0]),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","I-II","III-IV","Sum","I-II%","III-IV%")
print(xtable(x[order(x[,5]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$AMIn, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$AMIn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","NA","Sum","Non%","Oui%")
print(xtable(x[order(x[,6]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue[!global$STDAMIn==0], global$STDAMIn[!global$STDAMIn==0], useNA = "no")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue[!global$STDAMIn==0], global$STDAMIn[!global$STDAMIn==0]),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","I-II","III-IV","Sum","I-II%","III-IV%")
print(xtable(x[order(x[,5]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$AMPn, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$AMPn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","Sum","Non%","Oui%")
print(xtable(x[order(x[,5]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$avc_ait, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$avc_ait),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","NA","Sum","Non%","Oui%")
print(xtable(x[order(x[,6]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$VIHn, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$VIHn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","Sum","Non%","Oui%")
print(xtable(x[order(x[,5]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue[!global$VIHn==0], global$SIDAn[!global$VIHn==0], useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue[!global$VIHn==0], global$SIDAn[!global$VIHn==0]),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","Sum","Non%","Oui%")
print(xtable(x[order(x[,5]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$VHBn, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$VHBn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","Sum","Non%","Oui%")
print(xtable(x[order(x[,5]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
<<results="asis">>=
x <- addmargins(table(global$liste_longue, global$VHCn, useNA = "ifany")) %>% as.data.frame.matrix()
y <- round(prop.table(table(global$liste_longue, global$VHCn),1)*100,1) %>% as.data.frame.matrix()
x <- merge(x,y,by = "row.names")
colnames(x) <- c("Pathologie","Non","Oui","Sum","Non%","Oui%")
print(xtable(x[order(x[,5]),], digits = 1, auto = T), table.placement = "H", include.rownames = F)
@
...
y <- iga$TABACn %>%
  ...
  tidy()
full_join(x,y, by=".") %>%
  rename(Effectif = Freq.x, Pourcentage = Freq.y) %>%
  xtable() %>%
...
<<fig.height=3>>=
ggplot(iga, aes(dfg, fill = as.factor(PBRn))) +
...
<<fig.height=3.5>>=
ggplot(filter(global_greffe, liste_longue=="Néphropathie à dépôts d'IgA"), aes(dfg, age)) +
  geom_point() +
  geom_smooth() +
  facet_grid(.~greffe_METHOn)
@
...
<<fig.height=3.5>>=
summary(iga$ALBINI)
ggplot(iga[!is.na(iga$CIRHn),], aes(x = ALBINI)) +
  geom_histogram(aes(y= ..density..), col = "black", fill = "white") +
  geom_density() +
  facet_grid(CIRHn~., scales = "free_y") +
  labs(x = "Albuminémie (U/L)", title = "Albumine selon la présence d'une cirrhose")
@
...
<<results="asis">>=
x <- mutate(iga, gr_age_2 = cut(iga$age, breaks = c(0, 40, 99)))
table(x$gr_age_2, x$urg_rea) %>%
  prop.table(1) %>%
  "*"(100) %>%
  round(1) %>%
  xtable() %>%
...
<<>>=
chisq.test(table(x$gr_age_2, x$urg_rea), correct = F)
@
...
xtable(x, caption = "Pourcentage") %>% print(table.placement = "H")
@
...
\begin{multicols}{2}
<<results="asis">>=
x <- mutate(global_greffe, gr_age_2 = cut(global_greffe$age, breaks = c(0, 40, 99)))
for (i in levels(global_greffe$liste_longue)) {
  print(xtable(as.data.frame.matrix(round(prop.table(table(x$VAVn[x$liste_longue==i], x$gr_age_2[x$liste_longue==i]),2)*100,1)), caption = i), table.placement = "H")
}
@
\end{multicols}
...
par(mar = c(4,2,2,0))
global_greffe %>%
  survfit(Surv(kmdelaidc/365.25, kmdc)~liste_longue, data = .) %>%
  plot(main = "Courbe de survie", 
       xlab = "Temps (année)", 
       ylim = c(0.4,1),
       col = c("black", brewer.pal(n = 9, name = "Paired")))

legend("bottomleft", legend = c(levels(global_greffe$liste_longue)), lty = 1, col = c("black",brewer.pal(n = 9, name = "Paired")), cex = 1 , box.lwd = 0)
@
...
<<fig.height=3.5>>=
par(mar = c(2,2,2,0))
global_greffe[global_greffe$iga==1,] %>%
  survfit(Surv(kmdelaidc/365.25, kmdc)~1, data = .) %>%
  plot(ylim = c(0.8,1), main = "Courbe de survie Berger")
@

Cox sur age + sexe :
<<results="asis">>=
x <- coxph(Surv(kmdelaidc/365.25, kmdc)~age+sex, data = global_greffe[global_greffe$iga==1,])
print(xtable(x), table.placement = "H")
@

Cox sur age + sexe + dfg :
<<results="asis">>=
x <- coxph(Surv(kmdelaidc/365.25, kmdc)~age+sex+dfg, data = global_greffe[global_greffe$iga==1,])
print(xtable(x), table.placement = "H")
@

Cox sur age + sexe + dfg + albini :
<<results="asis">>=
x <- coxph(Surv(kmdelaidc/365.25, kmdc)~age+sex+dfg+ALBINI, data = global_greffe[global_greffe$iga==1,])
print(xtable(x), table.placement = "H")
@
...
La table "greffe" concernant la maladie de Berger contient \Sexpr{table(duplicated(greffe.iga$NEFG[!is.na(greffe.iga$GRF1)]))["FALSE"]} patients uniques après suppression de ceux qui n'avaient pas de maladie de Berger, de néphropathie à IgA ou de date de greffe. \Sexpr{length(intersect(global$RREC_COD,greffe.iga$NEFG[!is.na(greffe.iga$GRF1)]))} (\Sexpr{round(length(intersect(global$RREC_COD,greffe.iga$NEFG))/table(duplicated(greffe.iga$NEFG[!is.na(greffe.iga$GRF1)]))["FALSE"]*100,1)}\%) sont en commun avec la table "globale".
~\\

Dans la table globale avec la table greffe pour le Berger :

<<results="asis">>=
x <- global_greffe2[global_greffe2$iga==1 & !is.na(global_greffe2$iga),] %>% 
  group_by(!is.na(dgrf),
           !is.na(GRF1)) %>% 
  summarise(n()) %>%
  as.data.frame()
colnames(x) <- c("Date de greffe (global)","Greffe 1 (table greffe)", "Total")
print(xtable(x, digits = 0, align = c("c","c","c","c")), table.placement = "H", include.rownames = F)
@
...
\begin{multicols}{2}
<<results="asis">>=
x <- left_join(tidy(addmargins(table(global_greffe2$annee1[global_greffe2$iga==1 & !is.na(global_greffe2$iga)]))), tidy(round(prop.table(table(global_greffe2$annee1[global_greffe2$iga==1 & !is.na(global_greffe2$iga)]))*100,1)), by = "Var1")
colnames(x) <- c("Greffe 1","Fréquence","Pourcentage")
print(xtable(x, digits = 1, auto = T), table.placement = "H", include.rownames = F)

x <- left_join(tidy(addmargins(table(global_greffe2$annee2[global_greffe2$iga==1 & !is.na(global_greffe2$iga)]))), tidy(round(prop.table(table(global_greffe2$annee2[global_greffe2$iga==1 & !is.na(global_greffe2$iga)]))*100,1)), by = "Var1")
...
<<results="asis">>=
do.call("rbind", tapply(global_greffe$DELAIARF1, global_greffe$liste_longue, summary)) %>%
  xtable(digits = 1, auto = T) %>%
  print(table.placement = "H")
@

<<fig.height=2>>=
for (i in levels.default(global_greffe$liste_longue)) {
print(ggplot(global_greffe[global_greffe$liste_longue==i,], aes(DELAIARF1/12)) + 
  geom_histogram(aes(y=..density..), fill = "white", col = "black", bins = 13) +
  geom_density(fill = "red", alpha = 0.25, col = "red") +
  labs(x = 'Temps (en année)', title = i))
}
@
...
<<results="asis">>=
for (i in levels(global_greffe$liste_longue)){
x <- full_join(tidy(sort(table(global_greffe$ECHEC1[global_greffe$liste_longue==i], useNA = "always"), decreasing = T)[-1]), tidy(round(prop.table(sort(table(global_greffe$ECHEC1[global_greffe$liste_longue==i]), decreasing = T))*100,1)), by = "Var1")
colnames(x) <- c(i,"Frequence","Pourcentage")
x <- x[x$Frequence!=0,]
print(xtable(x, auto = T), table.placement = "H", include.rownames = F)
}
@
...
<<results="asis">>=
table(global_greffe$liste_longue, global_greffe$ECHEC2) %>%
  as.data.frame.matrix() %>%
  rename(`Compli vasc greffon` = `Complications vasculaires du greffon`) %>%
  xtable() %>%
  print(table.placement="H")
@
...
Maladie de Berger : \Sexpr{table(!is.na(global_greffe$GRF1[global_greffe$iga==1]), !is.na(global_greffe$ARF1[global_greffe$iga==1]))[2,2]} (\Sexpr{round(prop.table(table(!is.na(global_greffe$GRF1[global_greffe$iga==1]), !is.na(global_greffe$ARF1[global_greffe$iga==1]))[2,])*100,1)[2]}\%)

<<results="asis">>=
global_greffe %>% 
  filter(!is.na(GRF1), iga==1) %>%
  group_by(anirt, !is.na(ARF1)) %>% 
  summarise (n = n()) %>%
  mutate(Pourcentage = round(n/sum(n)*100,1)) %>%
  select(anirt, `!is.na(ARF1)`, Pourcentage) %>%
  spread(anirt, Pourcentage) %>%
  rename("Arrêt greffon" = `!is.na(ARF1)`) %>%
  xtable(auto = T) %>%
  print(include.rownames = F, table.placement = "H")
@

Toutes les pathologies : \Sexpr{table(!is.na(global_greffe$GRF1), !is.na(global_greffe$ARF1))[2,2]} (\Sexpr{round(prop.table(table(!is.na(global_greffe$GRF1), !is.na(global_greffe$ARF1))[2,])*100,1)[2]}\%)
...
<<results="asis">>=
do.call("rbind", tapply(global_greffe$DELAIARF1[global_greffe$iga==1], global_greffe$ECHEC1[global_greffe$iga==1], summary)) %>%
  xtable(digits = 1, auto = T) %>%
  print(table.placement = "H")
@


% Pourcentage d'arrêt de greffon/greffe/an pour la table greffe : \Sexpr{table(!is.na(greffe$GRF1), !is.na(greffe$ARF1))[2]} (\Sexpr{round(prop.table(table(!is.na(greffe$GRF1), !is.na(greffe$ARF1)))*100,1)[2]}\%)
% 
% <<>>=
% for (i in 2010:2014) {
%   print(i)
...
Nombre de patients ayant une date d'inscription (Berger) : \Sexpr{table(!is.na(global_greffe$DINSCMED[global_greffe$iga==1]))[2]} (\Sexpr{round(prop.table(table(!is.na(global_greffe$DINSCMED[global_greffe$iga==1])))*100,1)[2]}\%)

\begin{multicols}{2}
<<results="asis">>=
for (i in levels(global_greffe$liste_longue)) {
table(!is.na(global_greffe$DINSCMED[global_greffe$liste_longue==i]), global_greffe$greffe[global_greffe$liste_longue==i]) %>%
  prop.table(.,1) %>%
  `*`(100) %>%
  as.data.frame.matrix(row.names = c("Non inscrit","Inscrit")) %>%
  rename(`Non greffé` = `0`, `Greffé` = `1`)  %>%
  xtable(digits = 1, auto = T, caption = i) %>%
  print(table.placement = "H")
}
@
\end{multicols}
...
- Berger : \Sexpr{length(global_greffe$DECES1[!is.na(global_greffe$DECES1) & global_greffe$iga==1])}/\Sexpr{length(global_greffe$DECES1[global_greffe$iga==1 & !is.na(global_greffe$GRF1)])} (\Sexpr{round(length(global_greffe$DECES1[!is.na(global_greffe$DECES1) & global_greffe$iga==1])/length(global_greffe$DECES1[global_greffe$iga==1 & !is.na(global_greffe$GRF1)])*100,1)}\%) patients sont décédés au cours de la greffe 1.

- Diabète : \Sexpr{length(global_greffe$DECES1[!is.na(global_greffe$DECES1) & global_greffe$diab==1])}/\Sexpr{length(global_greffe$DECES1[global_greffe$diab==1 & !is.na(global_greffe$GRF1)])} (\Sexpr{round(length(global_greffe$DECES1[!is.na(global_greffe$DECES1) & global_greffe$diab==1])/length(global_greffe$DECES1[global_greffe$diab==1 & !is.na(global_greffe$GRF1)])*100,1)}\%) patients sont décédés au cours de la greffe 1.

- PKRD : \Sexpr{length(global_greffe$DECES1[!is.na(global_greffe$DECES1) & global_greffe$pkrd==1])}/\Sexpr{length(global_greffe$DECES1[global_greffe$pkrd==1 & !is.na(global_greffe$GRF1)])} (\Sexpr{round(length(global_greffe$DECES1[!is.na(global_greffe$DECES1) & global_greffe$pkrd==1])/length(global_greffe$DECES1[global_greffe$pkrd==1 & !is.na(global_greffe$GRF1)])*100,1)}\%) patients sont décédés au cours de la greffe 1.

- GNC : choisir els pathologies intégrées
%\Sexpr{length(global_greffe$DECES1[!is.na(global_greffe$DECES1) & global_greffe$gnc==1])}/\Sexpr{length(global_greffe$DECES1[global_greffe$gnc==1 & !is.na(global_greffe$GRF1)])} (\Sexpr{round(length(global_greffe$DECES1[!is.na(global_greffe$DECES1) & global_greffe$gnc==1])/length(global_greffe$DECES1[global_greffe$gnc==1 & !is.na(global_greffe$GRF1)])*100,1)}\%) patients sont décédés au cours de la greffe 1.

<<fig.height=5>>=
par(mar = c(4,4,2,0))
global_greffe[!global_greffe$gnc==1,] %>%
  survfit(Surv(kmdelaidc1/365.25, kmdc1)~liste_longue, data = .) %>%
  plot(main = "Courbe de survie", 
       xlab = "Temps (année)", 
       ylim = c(0.85,1),
       col = c("black", brewer.pal(n = 2, name = "Paired")))

legend("bottomleft", legend = c(levels(factor(global_greffe$liste_longue[!global_greffe$gnc==1]))), lty = 1, col = c("black",brewer.pal(n = 2, name = "Paired")), cex = 1 , box.lwd = 0)
@


<<fig.height=5>>=
global_greffe %>%
  survfit(Surv(kmdelaidc1/365, kmdc1)~gr_METHOn, data = .) %>%
  plot(main = "Courbe de survie, décès au cours de la greffe 1 (toute pathologie)\n selon le premier traitement de suppléance",
       xlab = "Temps (années) après la greffe",
       ylab = "Pourcentage",
       col = c("black","red"),
       ylim = c(0.9,1))
text(1,0.96, "Dialyse")
text(3.5,0.98, "Greffe")
@


<<fig.height=5>>=
global_greffe %>%
  survfit(Surv(kmdelaidc1/365, kmdc1)~greffe_METHOn, data = .) %>%
  plot(main = "Courbe de survie, décès au cours de la greffe 1 (toute pathologie)\n selon le premier traitement de suppléance",
       xlab = "Temps (années) après la greffe",
       ylab = "Pourcentage",
       col = c("black","red"),
       ylim = c(0.9,1))
text(1,0.96, "Dialyse")
text(4.5,0.98, "Greffe + greffe <1 an après dialyse")
@
