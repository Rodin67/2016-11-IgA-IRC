...
for (i in levels(global_greffe$liste_longue)) {
x <- full_join(
  tidy(sort(table(global_greffe$ECHEC1[global_greffe$liste_longue=="Diabète"], useNA = "always"), decreasing = T)[-1]), tidy(round(prop.table(sort(table(global_greffe$ECHEC1[global_greffe$liste_longue=="Diabète"]), decreasing = T))*100,1)), by = "Var1")
colnames(x) <- c(i,"Fréquence","Pourcentage")
...
Pourcentage d'arrêt de greffon/greffe/an maladie de Berger : \Sexpr{table(!is.na(global_greffe$GRF1[global_greffe$iga==1]), !is.na(global_greffe$ARF1[global_greffe$iga==1]))[2,2]} (\Sexpr{round(prop.table(table(!is.na(global_greffe$GRF1[global_greffe$iga==1]), !is.na(global_greffe$ARF1[global_greffe$iga==1]))[2,])*100,1)[2]}\%)
...
Pourcentage d'arrêt de greffon/greffe/an pour la table global : \Sexpr{table(!is.na(global_greffe$GRF1), !is.na(global_greffe$ARF1))[2,2]} (\Sexpr{round(prop.table(table(!is.na(global_greffe$GRF1), !is.na(global_greffe$ARF1))[2,])*100,1)[2]}\%)
