...
plotmeans(global$age~global$gr_bmi, gap=0, xlab = "", ylab = "Age lors IRCT")

global$gr_bmi <- relevel(global$gr_bmi, ref = "18.5-24.9")
summary(lm(global$age~global$gr_bmi))
@
...
<<>>=
addmargins(table(global$liste_longue, global$TABACn, useNA = "always"))
...
iga$dfg <- ifelse(iga$sex==1, 
                  ...
                  round(186.3*(iga$CREATINI/88.4)^-1.154 * (iga$age)^-0.203 * 0.742,2))
summary(iga$dfg)
hist(iga$dfg, xlab = "DFG (ml/min)", main = "DFG Berger")
@
